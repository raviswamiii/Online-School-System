      ...
      <div className="min-h-screen bg-[#ECF4E8] text-[#043915] flex flex-col overflow-auto">
        ...
        >
          <div className="flex justify-between items-center gap-5 bg-[#4C763B]/50 shadow-sm p-4 border border-[#B0CE88]/40">
            <div className="flex items-center gap-3 flex-1 min-w-0">
              ...
              <div className="text-white text-md sm:text-xl md:text-2xl font-semibold truncate">
        ...
        >
          <div className="bg-white shadow-sm p-6 border border-[#B0CE88]/40 h-full w-full max-w-full flex flex-col justify-center items-center overflow-hidden">
            <h2 className="text-[#4C763B] md:text-xl font-semibold text-center mb-4">
              About Us
            </h2>

            {schoolData?.aboutUs ? (
              <p className="md:text-xl text-gray-700 text-center leading-relaxed px-2 wrap-break-word break-all max-w-full">
                {schoolData.aboutUs}
              </p>
            ) : (
        ...
        >
          <div className="h-full w-full bg-white shadow-sm p-6 border border-[#B0CE88]/40 flex flex-col justify-center items-center">
            <h2 className="text-[#4C763B] md:text-xl font-semibold text-center mb-6">
              Meet Our Team
            </h2>
            ...
            {schoolData?.teamMembers?.length > 0 ? (
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
                {schoolData.teamMembers.map((member, idx) => (
                  <div
                    key={idx}
                    className="sm:w-[25vw] lg:w-[15vw] flex flex-col items-center p-3 shadow-md rounded-xl border border-[#B0CE88]/50"
                  >
                    <img
                      src={member.img}
        ...
        <footer className="bg-[#4C763B]/50 text-[#ECF4E8] pt-10 pb-6 px-6">
          <div className="max-w-6xl mx-auto">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 pb-8 border-b border-[#ECF4E8]/20">
              <div>
                <h3 className="font-semibold text-lg sm:text-2xl mb-4 text-[#FFE797] tracking-wide">
                  Contact Info
                </h3>
      ...
      {loggedInPrincipalId === principalId && (
        <div className="sticky bottom-0 w-full flex border-t border-[#B0CE88]/50 bg-white shadow-inner">
          <Link
            ...
            className="w-full text-center py-3 border-r border-[#B0CE88]/50 text-[#043915] md:text-lg font-semibold hover:bg-[#B0CE88]/30 transition-all"
          >
            Home
          </Link>
          ...
          <Link
            ...
            className="w-full text-center py-3 text-[#043915] md:text-lg font-semibold hover:bg-[#B0CE88]/30 transition-all"
          >
            My Profile
          </Link>
        </div>
